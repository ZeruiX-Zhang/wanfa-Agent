...
export function StatusBadge({ status, className }: { status: string; className?: string }) {
  ...
  const localized = translateDynamic(preferences.language, status);
  const label = localized || verificationLabel(status);
  return (
    <Badge
      className={cn(
        ...
        tone === "neutral" && "border-border bg-panel-muted text-foreground/80",
        className,
      )}
    >
      {label}
