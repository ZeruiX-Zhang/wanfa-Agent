...
from app.agent.analyzer import ResultAnalyzer
from app.agent.intent import detect_blocked_request
...
from app.sql.executor import ReadOnlySQLExecutor
from app.sql.generator import SQLPlanGenerator
...
class DataAnalystAgent:
    def __init__(self, settings: Settings | None = None):
        ...
        self.trace_store = TraceStore(self.settings)
    ...
    def run(self, request: DataAgentQueryRequest) -> DataAgentQueryResponse:
        ...
        if blocked:
            ...
            final_answer = "已拒绝执行：SQL validation failed。系统只允许基于业务数据库 schema 的只读 SELECT 查询，不读取本地文件或密钥。"
            return self._finish(
                run_id=run_id,
                trace_id=trace_id,
                status="rejected",
        ...
        if not validation.is_valid or not validation.sanitized_sql:
            final_answer = "已拒绝执行：SQL validation failed。请改用只读 SELECT 分析问题。"
            return self._finish(
                run_id=run_id,
                trace_id=trace_id,
                status="rejected",
        ...
        if execution.error:
            final_answer = f"SQL 执行失败：{execution.error}"
            return self._finish(
                run_id=run_id,
                trace_id=trace_id,
                status="failed",
    ...
    def _finish(
        ...
        *,
        ...
        return DataAgentQueryResponse(
            ...
            trace_url=f"/data-agent/runs/{run_id}" if request.include_trace else None,
            created_at=created_at,
            finished_at=finished_at,
        )
