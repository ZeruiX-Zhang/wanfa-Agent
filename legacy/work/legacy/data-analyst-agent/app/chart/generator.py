...
from app.schemas.data_agent import ChartResult, ChartSpec, SQLExecutionResult, SQLPlan
...
class ChartGenerator:
    ...
    def generate(self, run_id: str, plan: SQLPlan, execution: SQLExecutionResult) -> ChartResult:
        ...
        if not spec.x_column or not spec.y_column:
            return ChartResult(chart_type=spec.chart_type, generated=False, chart_error="缺少可绘图字段。")

        rows = execution.rows
        if not rows:
            return ChartResult(chart_type=spec.chart_type, generated=False, chart_error="结果为空，未生成图表。")
        ...
        try:
            self._draw_with_matplotlib(output_path, rows, spec)
        except Exception:
            try:
                self._draw_fallback_png(output_path, rows, spec)
            except Exception as exc:
                return ChartResult(
                    chart_type=spec.chart_type,
                    generated=False,
                    chart_error=f"图表生成失败：{exc}",
                )

        return ChartResult(
            chart_type=spec.chart_type,
            chart_path=self._display_path(output_path),
            chart_url=f"/data-agent/charts/{output_path.name}",
            generated=True,
        )
    ...
    def _infer_spec(self, plan: SQLPlan, execution: SQLExecutionResult) -> ChartSpec:
        ...
        if plan.chart_type == "none":
            return ChartSpec(chart_type="none", title="无图表")
        if "quarter" in columns and "total_revenue" in columns:
            return ChartSpec(chart_type="line", x_column="quarter", y_column="total_revenue", title="2025 年季度营收趋势")
        if "growth_rate" in columns:
            return ChartSpec(chart_type="bar", x_column="region", y_column="growth_rate", title="区域收入增长率")
        if "conversion_rate" in columns:
            return ChartSpec(chart_type="bar", x_column="channel", y_column="conversion_rate", title="渠道转化率")
        if "roi" in columns:
            return ChartSpec(chart_type="bar", x_column="channel", y_column="roi", title="渠道投放 ROI")
        if "gross_margin" in columns:
            return ChartSpec(chart_type="bar", x_column="product_line", y_column="gross_margin", title="产品线毛利率")
        if "avg_satisfaction_score" in columns:
            return ChartSpec(chart_type="bar", x_column="category", y_column="avg_satisfaction_score", title="工单类别满意度")
        if "total_revenue" in columns:
            ...
            return ChartSpec(chart_type="bar", x_column=x_col, y_column="total_revenue", title="收入对比")
        if "new_customers" in columns:
            return ChartSpec(chart_type="bar", x_column="region", y_column="new_customers", title="新增客户区域分布")
        numeric_col = self._first_numeric_column(execution.rows, columns)
        x_col = columns[0] if columns else None
        return ChartSpec(chart_type=plan.chart_type, x_column=x_col, y_column=numeric_col, title=question[:40])
    ...
    def _draw_with_matplotlib(output_path: Path, rows: list[dict[str, Any]], spec: ChartSpec) -> None:
        ...
        if spec.chart_type == "line":
            ax.plot(labels, values, marker="o", linewidth=2.4, color="#2563eb")
