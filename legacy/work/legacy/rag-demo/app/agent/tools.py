...
from app.security.path_guard import PathGuardError, ensure_within_allowed_path
...
def _analyze_csv(path: str, column: str) -> dict[str, Any]:
    ...
    answer = (
        f"{csv_path.name} 包含列 {', '.join(columns)}，共 {len(rows)} 行；"
        f"{column} 收入均值 {mean_value:g}，最大值 {max_value:g}，最小值 {min_value:g}。"
    )
    return {
