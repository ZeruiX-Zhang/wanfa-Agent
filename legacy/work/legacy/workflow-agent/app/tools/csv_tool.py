...
from app.schemas.agent import CSVAnalysisResult
from app.security.policies import ensure_inside_finance_dir
...
def analyze_csv(path: str | Path | None = None) -> CSVAnalysisResult:
    ...
    logic = (
        "按 quarter 聚合 revenue 求和、gross_margin 求均值、customer_count 求和；"
        "按 region 做同样聚合；区域增长率=(最后季度营收-首季度营收)/首季度营收。"
    )
    return CSVAnalysisResult(
        ...
        fastest_growth_region=fastest_growth_region,
